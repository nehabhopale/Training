...
func (p *passConnector) RegisterPassportRoutes(authRoute *mux.Router, nonAuthRoute *mux.Router) {
	nonAuthRoute.HandleFunc("/passports", p.getPassports).Methods("GET")
	nonAuthRoute.HandleFunc("/passports/{id}", p.DeletePassport).Methods("DELETE")
	nonAuthRoute.HandleFunc("/users/{id}/passports", p.GetPassportByUserId).Methods("GET")
	// nonAuthRoute.HandleFunc("/passports/{id}", p.updatePassport).Methods("PUT")
